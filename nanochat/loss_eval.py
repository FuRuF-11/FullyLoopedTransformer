...
def evaluate_ppl(model, batches):
    ...
    for x, y in batches:
        ...
        if (y.int() < 0).any():  # mps 目前对 int64 不支持 < 0 操作，只支持 int32
            # 存在 ignore_index（如 -1）的情况
            # 任何 target < 0 的 token 应被忽略
            valid = y >= 0
            # 只累加有效位置的损失
